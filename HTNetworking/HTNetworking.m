...
+ (void)getWithParams:(NSMutableDictionary *)params url:(NSString *)url result:(void(^)(id))resultBlock{
    [HTNET.manager GET:[self disposeUrl:url] parameters:params  progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        // 处理成功的模型
...
+ (RACSignal *)postWithParams:(NSMutableDictionary *)params url:(NSString *)url{
    ...
    return ([RACSignal createSignal:^RACDisposable * _Nullable(id<RACSubscriber>  _Nonnull subscriber) {
        // get请求
        [HTNET.manager POST:[self disposeUrl:url] parameters:params  progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
