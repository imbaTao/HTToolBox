...
// 重复组数的倍数
static NSInteger const RepeatMutiply = 10;
...
- (instancetype)initWithConfigModel:(HTCycleViewConfigModel *)configModel
{
    ...
    if (self) {
        ...
        self.cycleBoard.backgroundColor = [UIColor clearColor];
...
- (void)racConfig {
    ...
    [RACObserve(self.configModel, autoCycle) subscribeNext:^(id  _Nullable x) {
        ...
        if (self.configModel.autoCycle) {
            ...
            if (!self.timerSignal) {
                  // 创建一个定时器专门控制服务器时间
                  self.timerSignal = [[RACSignal interval:1 onScheduler:[RACScheduler mainThreadScheduler]] takeUntil:[self rac_signalForSelector:@selector(timerStop)]];
                
                  @weakify(self);
                  [self.timerSignal subscribeNext:^(NSDate * _Nullable x) {
                      @strongify(self);
...
- (void)setData:(NSArray *)data {
    ...
    if (data.count <= self.configModel.itemCountInOnePage) {
        ...
        self.configModel.autoCycle = true;
    }
...
- (void)dataRefresh {
    ...
    // 跳转到中间的那一组 为数据的数量 * 倍数 * 一半
    NSInteger  section = self.data.count * self.configModel.repeatMutiply * 0.5;
    
    // 定位这个下标占实际的那个位置
    NSInteger index = section % self.configModel.repeatMutiply % self.data.count;
    ...
    // 初始的一个下标
    self.configModel.currentIndexPath = [NSIndexPath indexPathForRow:0 inSection:section];
